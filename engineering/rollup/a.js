    ...
    window.initializeTTSDemo = function (localizedResources) {
        $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function textToSpeechVoiceListAjaxSuccess(data) {
                    ...
                    var sorted = data.sort(function (a, b) {
                        return `${ a.LocaleName }${ a.VoiceType }`.localeCompare(`${ b.LocaleName }${ b.VoiceType }`);
                    });
                    $.each(sorted, function (_index, element) {
                        ...
                        if (!voiceList[element.Locale]) {
                            ...
                            locales += '<option value="' + element.Locale + '">' + element.LocaleName + '</option>';
                        }
                    ...
                    const currentLocale = Object.keys(voiceList).find(lang => lang.toLowerCase() === localizedResources.locale?.toLowerCase());
                    if (currentLocale) language.value = currentLocale;
                    language.onchange();
                },
            ...
            function SpeakOnce(text) {
                ...
                synthesizer.speakSsmlAsync(text, function () { }, function (error) {
...
SpeakOnce(`<speak xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="http://www.w3.org/2001/mstts" xmlns:emo="http://www.w3.org/2009/10/emotionml" version="1.0" xml:lang="en-US"><voice name="zh-CN-XiaoxiaoNeural"><prosody rate="0%" pitch="0%">${e.data }</prosody></voice></speak>`)
})
...
var  a = window.open('https://azure.microsoft.com/zh-cn/products/cognitive-services/text-to-speech/#features')
a.postMessage(document.querySelector('.captions-text').innerText, '*')
